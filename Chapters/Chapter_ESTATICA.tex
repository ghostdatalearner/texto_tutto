...
cada red se generan $1000$ modelos nulos con los que se construyen las distribuciones de probabilidad de $NODF$ and $\overline k_{radius}$. El valor $zNODF$ de la red original es estadísticamente significativo si es mayor que $2$, ya que este índice crece con el anidamiento, en tanto que $z\overline k_{radius}$ debe ser menor que $-2$ para que se pueda afirmar que la red es compacta (figure \ref{fig:zscores}).
...
\subsection{Relación entre $degree$ y $k_{degree}$}

Las distribuciones de grado de las redes mutualistas siguen leyes de potencia truncada \cite{jordano2003invariant,vazquez2005degree}. El grado se utiliza de manera habitual como un indicador de importancia de la especie dentro de la red. Es muy sencillo de medir pero no está exento de inconvenientes.

La descomposición \textit{k-core} ha permitido definir el $k_{degree}$ como un grado ponderado por la distancia a la \textit{shell} máxima
\begin{figure}[h!]
\centering
\includegraphics[scale=0.5]{Figures/ESTATICA_kdegree_over_degree_M_PL_001.png}
\caption{Diagramas de dispersión que relacionan las \textit{k-magnitudes} con el tamaño de la red.}
\label{fig:ESTATICA_tamanyo_kdegree_kradius}
\end{figure}
