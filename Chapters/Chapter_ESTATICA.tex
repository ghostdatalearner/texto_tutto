...
cada red se generan $1000$ modelos nulos con los que se construyen las distribuciones de probabilidad de $NODF$ and $\overline k_{radius}$. El valor $zNODF$ de la red original es estadísticamente significativo si es mayor que $2$, ya que este índice crece con el anidamiento, en tanto que $z\overline k_{radius}$ debe ser menor que $-2$ para que se pueda afirmar que la red es compacta (figure \ref{fig:ESTATICA_zALL}).

\subsection{Experimento de recableado}

El objetivo de este experimento es medir la correlación de $NODF$ and $\overline k_{radius}$, para una red dada, cuando algunos de sus enlaces son recableados de forma aleatoria. Este procedimiento se aplica solo a las redes binarias, por la complejidad de definir un método de recableado para las pesadas que no altere por completo su estructura.
...
\begin{figure}[h!]
\centering
\includegraphics[scale=0.58]{Figures/ESTATICA_histo_corr_rewiring.png}
...
\begin{figure}[h!]
\centering
\includegraphics[scale=0.45]{Figures/ESTATICA_destruction_comparativa_dosredes.png}
...
\begin{figure}[h!]
\centering
\includegraphics[scale=0.5]{Figures/ESTATICA_destructions_comparison.png}
...
\begin{table}[ht!]
\fontsize{2.1mm}{2.1mm}\selectfont
  \centering
    \begin{tabular}{lrrrrrrrrr}
    \toprule
    $Red$  & $Matriz$ & $Plantas$ & $Animales$ & $Enlaces$ & $k_{max}$ & $\overline k_{degree}$ & $\overline k_{radius}$ & $NODF$ & $Modularity$ \\
    \midrule
