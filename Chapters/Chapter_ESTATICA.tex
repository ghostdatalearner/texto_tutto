...
La medida del anidamiento es la más popular en el análisis del mutualismo, pero no está exenta de problemas. El principal es que existen distintos indicadores que no miden lo mismo. No vamos a hacer una revisión de todos ellos, porque la producción en este terreno ha sido prolífica \cite{ulrich2009consumer}, pero mencionaremos los más destacados. 
...
El  $\overline k_{degree}$ recuerda el \textit{índice de Harary} que se deine como la suma de los inversos de las distancias entre todos los nodos de un grafo conectado \cite{plavvsic1993harary}. El  $\overline k_{degree}$ teniendo solo tiene en cuenta los enlaces con la \textit{shell} máxima y utiliza el inverso de los $k_{radius}$, no de las distancias.
...
si se aleja al menos dos desviaciones estándar de la media estimada. En general se utilizarán unidades reducidas en este procecidimiento ($z scores$).
...
cada red se generan $1000$ modelos nulos con los que se construyen las distribuciones de probabilidad de $NODF$ y $\overline k_{radius}$. El valor $zNODF$ de la red original es estadísticamente significativo si es mayor que $2$, ya que este índice crece con el anidamiento, en tanto que $z\overline k_{radius}$ debe ser menor que $-2$ para que se pueda afirmar que la red es compacta (figure \ref{fig:ESTATICA_zALL}).
...
Se empieza cambiando uno por ciento de los enlaces, seleccionados al azar sin reemplazo. Las \textit{k-magnitudes} de la red alterada se calculan y se almacenan. La operación se repite con el porcentaje ente $1$ y $50$ en $25$ pasos. El recableado se aplica siempre a la red original, no es acumulativo. 
...
\begin{figure}[h!]
\centering
\includegraphics[scale=0.7]{Figures/ESTATICA_z_M_PL_012_rewire_Binary_ES_model_5.png}
...
La medida de compacidad con$\overline {k}_{radius}$ es más restrictiva. De las $32$ redes solo una es significativamente compacta ($z\overline {k}_{radius} < 2$), la red de polinizadores $059$, que además es una de las tres que muestra $zNODF$ por encima de $2$.

\begin{figure}[h!]
\centering
\includegraphics[scale=0.4]{Figures/ESTATICA_zscores_all.eps}
...
\fontsize{2.5mm}{2.5mm}\selectfont
  \centering
    ...
    \bottomrule
    \end{tabular}%
    \caption{\label{table:table_results_zscores} Propiedades de las redes utilizadas en el estudio.}
\end{table}%
...
\fontsize{2.5mm}{2.5mm}\selectfont
  \centering
    ...
    \toprule
    $Red$  & $Especies$ & $Enlaces$ & $Corr$ & $Asimetría$ &      & $Red$  & $Especies$ & $Enlaces$ & $Corr$& $Asimetría$ \\
    \midrule
