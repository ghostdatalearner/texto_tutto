...
No obstante, un pequeño porcentaje se salía de este patrón, y vimos que eran redes pequeñas o con una marcada asimetría. En los dos ejemplos que se incluyen a continuación, los diagramas zigurat permiten entener el por qué de este comportamiento.
...
En la red sin alterar, se observa un índice $k$ máximo de $4$, y una gran conectividad directa con esas \textit{shells}. Hay un importante número de especies en la $1$-$shell$. En el gráfico de correlación general entre $NODF$ y $\overline k_{radius}$ (figura \ref{ESTATICA_corrfigs}) esta red se sitúa en una zona intermedia.

Al recablear al azar seis enlaces, la estructura cambia poco, aunque puede verse como aparece una $2$-$shell$ de plantas y aumenta la conectividad entre las \textit{k shells} inferiores. Es importante recalcar que esta red alterada es resultado de una realización al azar, la configuración puede variar entre distintas realizaciones, por eso las gráficas de la figura \ref{ESTATICA_corrfigs} se obtuvieron repitiendo veinte veces. No obstante, un recableado mínimo, tendrá pocas consecuencias globales en en esta red.

Al aumentar a diecisiete los enlaces reconectados, la degradación es más notable. El índice $k$ máximo baja a $3$. Si sigue creciendo esa cifra, la modificación es aun más evidente, en la imagen para treinta y dos cambios al azar, las $2$-$shells$ han crecido a costa de la $3$ y tienen muchas más conexiones entre ellas. La $NODF$ ha bajado desde el original $30,40$ a $12,22$ y el $\overline k_{radius}$ pasa de $2,51$ a $2,82$. 

\begin{figure}[ht!]
\centering
\includegraphics[scale=0.75]{Figures/VIS_PL_012.pdf}
